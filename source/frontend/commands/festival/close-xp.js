...
module.exports = new SubcommandWrapper("close-xp", "End the festival, returning to normal XP",
	async function executeSubcommand(interaction, theater, isDevMode, logicLayer) {
		await theater.company.update({ "xpFestivalMultiplier": 1 });
		updateBotNicknameForFestival(await interaction.guild.members.fetchMe(), theater.company);
		interaction.reply(addCompanyAnnouncementPrefix(theater.company, { content: "The XP multiplier festival has ended. Hope you participate next time!" }));
