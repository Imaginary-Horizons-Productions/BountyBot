	...
	async function executeSubcommand(interaction, runMode, ...[logicLayer, posterId]) {
		...
		interaction.reply({
			...
			return interaction.awaitModalSubmit({ filter: incoming => incoming.customId === `${SKIP_INTERACTION_HANDLING}${collectedInteraction.id}`, time: timeConversion(5, "m", "ms") }).then(async modalSubmission => {
				...
				const bountyEmbed = await bounty.embed(modalSubmission.guild, poster.getLevel(company.xpCoefficient), false, company, await logicLayer.bounties.findBountyCompletions(bountyId));
